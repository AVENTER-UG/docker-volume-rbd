...
func (d *rbdDriver) MountRbdImage(imageName string) (err error, mountpoint string) {
	...
	device, err = d.mapImage(imageName)
	if err != nil {
		logrus.Errorf("unable to map: %s %s", imageName, err.Error())
		return err, ""
	}
	...
	if err != nil {
		...
		logrus.Errorf("unable to make mountpoint: %s %s", mountpoint, err.Error())
		return err, ""
	}

	// mount
	mountOptions := os.Getenv("MOUNT_OPTIONS")
	err = d.mountImage(device, imageName, mountOptions)
	if err != nil {
		defer d.FreeUpRbdImage(imageName)
